...
public class UsuariosFuncionalTest {
    ...
    public void testBuscarUsuarioPorIDComSucesso() {
        UsuariosResPojo usuarioCriado = buscarUsuarioPorID(cadastrarUsuario().get_id());
        UsuariosReqPojo usuarioReq = convertToUsuarioReqPojoGson(usuarioCriado);

        UsuariosResPojo usuarioBuscado =
        given()
            .log().all()
            .pathParam("_id", usuarioCriado.get_id())
        .when()
            .get("usuarios/{_id}")
        .then()
                .statusCode(200)
                ...
                .extract().as(UsuariosResPojo.class)
                ;
        Assertions.assertEquals(usuarioReq.getNome(), usuarioBuscado.getNome());
        Assertions.assertEquals(usuarioReq.getEmail(), usuarioBuscado.getEmail());
        Assertions.assertEquals(usuarioReq.getPassword(), usuarioBuscado.getPassword());
        Assertions.assertEquals(usuarioReq.getAdministrador(), usuarioBuscado.getAdministrador());
    }
    ...
    public void testDeletarUsuarioComSucesso() {
        ...
        ;
        UsuariosResPojo usuarioResult = buscarUsuarioPorID(usuarioCriado.get_id());
        Assertions.assertEquals("Usuário não encontrado", usuarioResult.getMessage());
        Assertions.assertNull(usuarioResult.get_id());
    }
